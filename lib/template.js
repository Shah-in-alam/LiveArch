...
const PORT = ${port};
const SNAPSHOT = ${opts.snapshot ? 'true' : 'false'}; // hosted static snapshot (no live watcher)
...
function connect() {
  // Hosted snapshot (permanent URL): show the stored diagram, no live socket.
  if (SNAPSHOT) { setStatus(true, 'Snapshot'); $('btnReview').style.display = 'none'; return; }
