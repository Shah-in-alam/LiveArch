...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .export-menu {
    ...
    display: none; flex-direction: column; min-width: 132px; padding: 5px;
  ...
  .export-menu button:hover { background: var(--lc-soft, rgba(0,0,0,.05)); }
  ...
  #review .rs .rissue:hover { border-color: var(--accent); background: var(--lc-soft, rgba(43,108,176,.08)); }
...
<header>
  ...
  <div class="toolbar">
    ...
    <button id="toggleInferred" title="Dashed edges are inferred guesses, not real imports">Inferred: on</button>
    ...
    <div class="export-wrap">
      <button id="btnExport" title="Download the diagram as an image">⤓ Export</button>
...
let showEdges = true;
let showLabels = true;
let showInferred = true; // dashed "guessed" edges (provenance !== 'import')
let selectedId = null;
let zoom = 1;
...
function buildDiagram() {
  ...
  for (const layerId of order) {
    ...
    // group nodes into subsystem boxes: by type, but generic modules split by
    // their source folder so the nesting mirrors the directory structure.
    const grps = layerGroups(nodes);
    ...
    if (grps.length > 1) {
      ...
      for (const g of grps) {
        ...
        head.innerHTML = iconSvg(g.iconType) + esc(g.label);
        sub.appendChild(head);
        const wrap = document.createElement('div');
        wrap.className = 'nodes';
        for (const n of g.nodes) wrap.appendChild(makeNode(n, color));
...
// Split a layer's nodes into ordered subsystem groups. Typed nodes group by
// type; generic 'module' nodes group by their source folder (directory).
function layerGroups(nodes) {
  const map = new Map();
  for (const n of nodes) {
    let key, label, iconType, sort;
    if (n.type === 'module' && n.file) {
      const dir = n.file.split('/').slice(0, -1).join('/') || '(root)';
      key = 'dir:' + dir; label = dir; iconType = 'folder'; sort = 100; // modules after typed groups
    } else {
      key = 'type:' + n.type; label = GROUP_LABELS[n.type] || n.type; iconType = n.type;
      const idx = TYPE_ORDER.indexOf(n.type); sort = idx < 0 ? 90 : idx;
    }
    if (!map.has(key)) map.set(key, { label, iconType, sort, nodes: [] });
...
function drawEdges() {
  ...
  for (const e of ARCH.edges) {
    const inferred = e.provenance && e.provenance !== 'import';
    if (inferred && !showInferred) continue; // hide guessed edges when toggled off
    ...
    // Inferred edges are heuristic guesses, not observed imports — draw them
    // dashed so real structure reads honestly against the graph's assumptions.
    if (inferred) path.setAttribute('stroke-dasharray', '5 4');
...
$('toggleInferred').onclick = () => {
  showInferred = !showInferred;
  $('toggleInferred').textContent = 'Inferred: ' + (showInferred ? 'on' : 'off');
  $('toggleInferred').classList.toggle('off', !showInferred);
  drawEdges();
};
$('toggleTheme').onclick = () => {
...
// ---- Export (PNG / SVG) --------------------------------------------------
// Serialise the live #canvas into a standalone SVG (page CSS embedded, theme
// colours inlined so dark/light both export faithfully). Everything on the
// canvas — nodes, brand logos, elbow connectors — is inline SVG/HTML with no
// external resources, so rasterising to PNG never taints the canvas.
const CSS_VARS = ['--bg','--grid','--panel','--card','--border','--text','--muted',
  '--accent','--line','--green','--red','--flash','--shadow','--shadow-sm'];

function safeName(ext) {
  const base = (ARCH.name || 'architecture').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
  return 'livearch-' + (base || 'architecture') + '.' + ext;
}
...
function buildExportSvg() {
  drawEdges(); // make sure connectors reflect the current toggle/state
  const NS_SVG = 'http://www.w3.org/2000/svg', NS_XHTML = 'http://www.w3.org/1999/xhtml';
  const canvas = $('canvas');
  const pad = 40;
  ...
  const grid = cs.getPropertyValue('--grid').trim() || 'transparent';
  ...
  // Pin the current theme's variable values so the embedded stylesheet resolves
  // to exactly what's on screen (the SVG root has no <body> to match .dark/.light).
  let vars = '';
  for (const v of CSS_VARS) vars += v + ':' + cs.getPropertyValue(v).trim() + ';';
  const wrap = document.createElementNS(NS_XHTML, 'div');
  wrap.setAttribute('xmlns', NS_XHTML);
  wrap.setAttribute('style',
    vars + 'box-sizing:border-box;width:' + W + 'px;padding:' + pad + 'px;' +
    'font-family:ui-sans-serif,system-ui,-apple-system,\\'Segoe UI\\',Roboto,sans-serif;' +
    'background-color:' + bg + ';background-image:radial-gradient(' + grid + ' 1.3px,transparent 1.3px);background-size:24px 24px;');

  const styleEl = document.createElementNS(NS_XHTML, 'style');
  const pageStyle = document.querySelector('style');
  styleEl.textContent = pageStyle ? pageStyle.textContent : '';
  wrap.appendChild(styleEl);

  const clone = canvas.cloneNode(true);
  clone.style.transform = 'none';
  clone.style.minWidth = '0';
  clone.removeAttribute('id'); // avoid duplicate ids in the exported doc
...
function exportPng() {
  let built;
  try { built = buildExportSvg(); }
  catch (e) { toast('Export failed: ' + (e && e.message || e)); return; }
  const scale = Math.min(2, window.devicePixelRatio || 1) < 1 ? 1 : 2;
  ...
  img.onload = () => {
    try {
      ...
      c.toBlob((b) => {
        ...
        if (b) { downloadBlob(b, safeName('png')); toast('Exported PNG'); }
        else { toast('PNG failed — exporting SVG instead'); exportSvg(); }
      }, 'image/png');
    } catch (err) {
      URL.revokeObjectURL(url);
      toast('PNG blocked — exporting SVG instead');
      exportSvg();
    }
  };
  img.onerror = () => {
    URL.revokeObjectURL(url);
    toast('PNG failed — exporting SVG instead');
    exportSvg();
  };
  img.src = url;
}
...
exportMenu.querySelectorAll('button').forEach((b) => {
  b.onclick = () => {
    ...
    if (b.dataset.fmt === 'png') exportPng(); else exportSvg();
  };
});
...
// Build a prefilled GitHub "new issue" URL for a review suggestion. The form
// opens pre-populated; the user reviews and submits it themselves.
function issueUrl(s, node) {
  const repo = ARCH.repo;
  let title = '[LiveArch] ' + (s.message || 'Architecture suggestion');
  if (title.length > 120) title = title.slice(0, 117) + '…';
  let body = '';
  if (node) body += '**Component:** ' + (node.label || node.id) + (node.file ? ' (\`' + node.file + '\`)' : '') + '\\n\\n';
  body += '**Concern:** ' + (s.message || '') + '\\n\\n';
  if (s.suggestion) body += '**Suggested fix:** ' + s.suggestion + '\\n\\n';
  body += '---\\n_Auto-generated by the LiveArch architecture review._';
  return repo.url + '/issues/new?title=' + encodeURIComponent(title) +
    '&body=' + encodeURIComponent(body) + '&labels=architecture';
}
$('btnReview').onclick = async () => {
  ...
  try {
    ...
    $('rList').innerHTML = items.map((s) => {
      ...
      // One-click: open a prefilled GitHub "new issue" form (only if this repo
      // has a GitHub remote). Nothing is submitted until the user clicks Submit.
      const issue = ARCH.repo ? '<a class="rissue" target="_blank" rel="noopener noreferrer" href="' +
        esc(issueUrl(s, node)) + '">⤴ Create GitHub issue</a>' : '';
      return '<div class="rs ' + (s.severity === 'warning' ? 'warning' : 'info') + '">' + head +
        '<div class="rmsg">' + esc(s.message) + '</div>' +
        (s.suggestion ? '<div class="rsug">' + esc(s.suggestion) + '</div>' : '') + issue + '</div>';
    }).join('');
  } catch (e) {
