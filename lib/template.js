...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .node.selected { background: var(--lc-soft); border-color: var(--lc, var(--accent)); box-shadow: 0 0 0 2px var(--lc, var(--accent)); }
  .node.dim { opacity: .22; }
  .node .tile {
  ...
  .node .deg {
    position: absolute; top: 2px; right: 8px; min-width: 17px; height: 17px; padding: 0 5px;
    border-radius: 9px; background: var(--lc, var(--line)); color: #fff; font-size: 10px;
    font-weight: 800; display: none; align-items: center; justify-content: center; box-shadow: var(--shadow-sm);
  }
...
function makeNode(n, color) {
  ...
  el.innerHTML =
    '<span class="tile">' + iconSvg(n.type) + '</span>' +
...
function selectNode(id) {
  ...
  $('pTitle').innerHTML = '<span class="tile" style="width:26px;height:26px;border:none;background:transparent;color:' + (LAYER_COLORS[node.layer] || '#888') + '">' + iconSvg(node.type) + '</span> ' + esc(node.label);
