...
$('btnReview').onclick = async () => {
  ...
  try {
    ...
    if (!res.ok) {
      $('rStatus').textContent = data.code === 'NO_API_KEY'
        ? 'Set ANTHROPIC_API_KEY to enable AI review (LiveArch Pro).'
        : ('Review failed: ' + (data.error || res.status));
      return;
    }
    const items = data.suggestions || [];
    $('rStatus').textContent = items.length ? items.length + ' suggestion(s)' : 'No architectural concerns found. ✓';
