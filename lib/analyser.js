...
function classifyFile(rel) {
  ...
  // TypeScript declaration files are type-only, not architecture — skip them.
  // (They otherwise became generic backend "module" nodes.)
  if (/\.d\.[cm]?ts$/i.test(file)) return null;

  // Tests — includes NestJS-style `*.e2e-spec.ts` (the `-spec` variant).
  if (/[.-](test|spec)\.[jt]sx?$/.test(file)) {
    return { id: `file-${rel}`, label: name, icon: '🧪', type: 'test', layer: LAYER.TOOLING, file: rel };
  }

  // Storybook stories are dev artefacts, not architecture — skip.
  if (/\.stories\.(?:[jt]sx?|mdx)$/i.test(file)) return null;

  // Tooling / config files
  if (/^vite\.config\.[jt]s$/.test(file)) {
    return { id: 'dep-vite', label: 'Vite', icon: '⚡', type: 'tooling', layer: LAYER.TOOLING, file: rel };
  }
  // Other build/tool config (next.config, tailwind.config, jest.config, …) is
  // not architecture — skip. (Vite is handled just above / via its dep node.)
  if (/\.config\.[mc]?[jt]s$/i.test(file)) return null;
  ...
  // Entry points — main/index/app at the repo root or one level deep only.
  // Deeper `index.ts` files are barrel/re-export files, not app entries.
  if (isCode && /^(main|index|app)$/i.test(name) && dirs.length <= 1) {
...
// `export * as ns from './y'`. Requires the ` from `, so it never matches a
// value like `export const x = './y'`.
const EXPORT_FROM_RE = /export\s+[^'";]*?\sfrom\s+['"]([^'"]+)['"]/g;
...
function parseImports(absPath, aliases = null) {
  ...
  const specs = new Set();
  for (const re of [IMPORT_RE, REQUIRE_RE, DYNAMIC_RE, EXPORT_FROM_RE]) {
    re.lastIndex = 0;
    let m;
    while ((m = re.exec(src))) {
      const s = m[1];
...
function resolveToKnown(target, relSet) {
  ...
  // NodeNext/ESM: a `.js`/`.jsx`/`.mjs`/`.cjs` specifier often points at a
  // TypeScript source file (`import './foo.js'` → `foo.ts`). Retry the base
  // without the JS extension. Real .js files already matched as-is above.
  const jsExt = target.match(/\.(?:js|jsx|mjs|cjs)$/i);
  if (jsExt) {
    const base = target.slice(0, -jsExt[0].length);
    for (const ext of CODE_EXT) {
      if (relSet.has(base + ext)) return base + ext;
    }
  }
  return null;
}
...
/**
 * Strip `//` and block comments from JSONC, respecting string literals — so a
 * `/*` inside a path value like `"./src/*"` is NOT mistaken for a comment
 * (which corrupted the whole file under the old regex approach).
 */
...
/** Parse JSON that may contain a BOM, comments, or trailing commas (tsconfig is JSONC). */
...
/**
 * Read a tsconfig/jsconfig and its `extends` chain, returning the merged
 * `compilerOptions`. Base configs are applied first, then this file overrides
 * them (matching TypeScript; `paths` is replaced, not deep-merged). Only
 * relative/file `extends` are followed — package-name extends (e.g.
 * `@tsconfig/node18`) would need node_modules resolution and are skipped.
 */
function readCompilerOptions(file, seen) {
  if (seen.has(file) || seen.size > 10) return null; // cycle / depth guard
  seen.add(file);
  let raw;
  ...
  const exts = Array.isArray(ext) ? ext : (ext ? [ext] : []);
  for (const e of exts) {
    if (typeof e !== 'string' || !e.startsWith('.')) continue; // relative extends only
    let ep = path.resolve(path.dirname(file), e);
    if (!/\.json$/i.test(ep)) ep += '.json';
    const baseCo = readCompilerOptions(ep, seen);
    if (baseCo) co = { ...co, ...baseCo };
  }
  if (json.compilerOptions && typeof json.compilerOptions === 'object') {
    co = { ...co, ...json.compilerOptions };
  }
  return co;
}
 ...
 * root (following `extends` chains). Returns { baseDir, paths:[{ prefix,
 * suffix, hasStar, targets }] } or null when none is configured. `baseDir` is
 * `compilerOptions.baseUrl` (default '.') as a POSIX rel path; each target
 * keeps its own prefix/suffix around the `*` wildcard so a captured segment
 * can be substituted.
 */
function loadAliases(root) {
  for (const name of ['tsconfig.json', 'jsconfig.json']) {
    const co = readCompilerOptions(path.join(root, name), new Set());
...
// `api` is intentionally excluded — it's overwhelmingly a client-side axios/
// fetch wrapper (`api.get('/users')`), so including it flagged client HTTP
// calls as fake server routes.
const ROUTE_RE = /\b(?:app|router|fastify|server)\s*\.\s*(get|post|put|patch|delete|options|head|all)\s*\(\s*['"\`]([^'"\`]+)['"\`]/gi;
