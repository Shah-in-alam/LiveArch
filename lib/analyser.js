...
function classifyFile(rel) {
  ...
  // Python source files have their own conventions.
...
function buildPrismaGraph(root, files) {
  ...
  for (const abs of files) {
    ...
    for (const model of parsePrismaModels(abs)) {
      ...
      edges.push({ from: 'dep-prisma', to: id, label: 'defines' });
    }
  }
  return { nodes, edges };
}
...
function analyse(root, files = [], opts = {}) {
  ...
  // Python tech stack (requirements/pyproject/setup.py) and Prisma models.
