...
export async function GET(req) {
  ...
  return Response.json({ tokens: await listTokens(acc.id) });
}
...
export async function POST(req) {
  ...
  const token = await issueToken(acc.id, (body && body.name) || 'token');
...
export async function DELETE(req) {
  ...
  const ok = await revokeToken(acc.id, body && body.tokenHash);
